...
/broadcast — создать рассылку (пошагово: интервал → старт → конец → режим → окно → контент).
/broadcast_list – список активных рассылок (ID, статус, режим, окно, next, end).
/broadcast_stop <id> – остановить рассылку определённого поста по айди.
/broadcast_mode <id> <full|limited> – установить посту по айди режим публикации 24/7 или дневной.
/broadcast_window <id> <HH:MM-HH:MM> – установить локальное дневное окно рассылки для определённого поста по айди.
...
full – публикация 24/7.
limited – только внутри установленного временного окна.
- Окно - время в течение суток, когда разрешена публикация постов-рассылок, если у них не задана публикация 24/7.
Если текущее время вне окна, пост не публикуется.
- Локальное через /broadcast_window. Локальное окно устанавливается для каждого поста отдельно. Локальное окно
имеет приоритет над глобальным.
- Глобальное через /broadcast_global_window. Работает для всех постов без установленного локального окна.
Пример: 
Посту 1 установлено локальное окно 09:00-18:00, посту 2 окно не установлено. Глобальное окно 10:00-17:00.
Пост 1 будет публиковаться с 09:00 до 18:00, пост 2 - с 10:00 до 17:00.
